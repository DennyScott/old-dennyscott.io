...
exports.createPages = ({ graphql, actions }) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const mdxPostTemplate = path.resolve('./src/templates/MdxPostTemplate.js');
    resolve(
      graphql(`
        {
      ...
      graphql(`
        {
          allMdx(filter: { fileAbsolutePath: { regex: "//posts//" } }, limit: 1000) {
            edges {
              node {
                frontmatter {
                  slug
      ...
      `).then(result => {
        ...
        _.each(result.data.allMdx.edges, edge => {
          ...
          createPage({ path: slug, component: mdxPostTemplate, context: { slug: slug } });
        });
      }),
    );
  });
